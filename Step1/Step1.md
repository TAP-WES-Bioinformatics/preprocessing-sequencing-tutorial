...
We will then map these read to a reference genome (sequence.fasta) using the tool ```minimap2`` and pass it to ```samtools``` to compress it into a bam file. The output is mapped, compressed reads are unsorted and unindexed. 
