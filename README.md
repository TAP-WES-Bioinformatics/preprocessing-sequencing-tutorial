...
Once the container opens, it will take another few minutes for the ```postCommand``` to run, which creates the conda enviornment and installs our dependencies. You can check if this is finished by typing ```conda env list``` and seeing if you see ```preprocessing-sequencing-tutorial``` in the enviornment lists.
